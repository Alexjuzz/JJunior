...
public class QueryBuilder {
     ...
     * @return
     */
    public String buildDeleteQuery(Object obj, UUID primaryKey) throws IllegalAccessException {
        Class<?> clazz = obj.getClass();
        ...
        if (clazz.isAnnotationPresent(Table.class)) {
            ...
            for (Field field : fields) {
                if (field.isAnnotationPresent(Column.class)) {
                    field.setAccessible(true);
                    Column columnAnnotation = field.getAnnotation(Column.class);
                    if(primaryKey.equals(field.get(obj))){
                    if(columnAnnotation.primaryKey()){
                        query.
                                append("= '").
                                append(field.get(obj))
                        ...
                        query = new StringBuilder(String.format("Пользолватель c таким id : '%s' не найден.", primaryKey));
                    }
                }
